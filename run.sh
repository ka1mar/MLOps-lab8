...
kubectl -n foodfacts wait --for=condition=ready pod -l app=mysql --timeout=3000s
...
kubectl -n foodfacts wait --for=condition=ready pod -l app=spark-master --timeout=3000s

# Wait for Spark Worker
echo "Waiting for Spark Worker to start..."
kubectl -n foodfacts wait --for=condition=ready pod -l app=spark-worker --timeout=3000s
